	...
	public class ChessConsoleView : IConsoleView {
		...
		public string MoveToString(ChessMove move) {
			if (move.MoveType.Equals(ChessMoveType.PawnPromote))
				return "(" + PositionToString(move.StartPosition) + "," + PositionToString(move.EndPosition) + "," + move.promoteTo + ")";
			else
		...
		public ChessMove ParseMove(string moveText) {
			...
			moveText = moveText.Replace(" ", String.Empty);
			moveText = moveText.Replace(",", String.Empty);
			moveText = moveText.Replace("(", String.Empty);
			moveText = moveText.Replace(")", String.Empty);
			string start_s = moveText.Substring(0, 2);
			string end_s = moveText.Substring(2, 2);
			BoardPosition start = ParsePosition(start_s);
			BoardPosition end = ParsePosition(end_s);
			if (moveText.Length > 4)
			{
				char piece_option = moveText[4];
				if (piece_option.Equals('k'))
					return new ChessMove(start, end, ChessPieceType.Knight,ChessMoveType.PawnPromote);
				else if (piece_option.Equals('q'))
					return new ChessMove(start, end, ChessPieceType.Queen, ChessMoveType.PawnPromote);
				else if (piece_option.Equals('b'))
					return new ChessMove(start, end, ChessPieceType.Bishop, ChessMoveType.PawnPromote);
				else if (piece_option.Equals('r'))
